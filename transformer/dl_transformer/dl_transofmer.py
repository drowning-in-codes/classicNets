...
#   author:proanimer
#   createTime:2024/6/14 上午10:36
#   lastModifiedTime:2024/6/14 上午10:36
#   file:dl_transofmer.py
...
class AddNorm(nn.Module):
    def __init__(self, normalized_shape, out, **kwargs):
        super(AddNorm, self).__init__(**kwargs)
        self.norm = nn.LayerNorm(normalized_shape)
        self.out = out

    def forward(self, x, y):
        return self.ln(self.dropouu(y) + x)
...
class DotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, valid_lens):
        attention = torch.bmm(q, k.transpose(1, 2)) / math.sqrt(q.shape[-1])
        if valid_lens is not None:
            attention = attention.masked_fill(valid_lens == 0, -1e9)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, valid_lens):
        y = self.addnorm2(x, self.attention(x, x, x, valid_lens))
...
class DecoderBlock(nn.Module):
    def __init__(self, key_size, query_size, value_size, num_hiddens, norm_shape, ffn_num_input, ffn_num_hiddents,
        ...
        self.ffn = PositionWiseFFN(ffn_num_input, ffn_num_hiddents, num_hiddens)
