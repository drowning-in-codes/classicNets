# from https://gist.github.com/nreHieW/a4ae05d216c5326c9fb9a70fcdda3274
...
def rope(x: torch.Tensor, freqs_cis: tuple[torch.Tensor, torch.Tensor]):
    def rotate(x):
        """
...
def attn(
    ...
    if sliding_window_size is not None:  # Sliding window attention
        ...
        sliding_mask = torch.triu(all_ones, -1 * sliding_window_size + 1) * torch.tril(all_ones,
                                                                                       sliding_window_size - 1)
        mask = torch.where(torch.tensor(sliding_mask == 1), mask, torch.tensor(-float("inf")))
...
def precompute_freqs_cis(head_dim: int, seq_len: int, base_theta: float = 500000.0):
    inv_freqs = 1.0 / (base_theta ** (
            torch.arange(0, head_dim, 2).float() / head_dim))  # Eq 15: theta_{1} ... theta_{dim/2}. Shape: (dim/2)
    m = torch.arange(seq_len)  # all possible position indices
    freqs = torch.outer(m,
                        inv_freqs).float()  # [m_i * theta_j] for all i (positions) and j (frequencies). Shape: (seq_len, dim/2) | freqs[i][j] == m[i] * inv_freqs[j]
...
def transformer(in_tokens: torch.Tensor, weights: TransformerWeights):
    ...
    for i, layer in enumerate(weights.layers):
        ...
        hidden = attn(hidden, layer, freqs_cis, sliding_window_size=SLIDING_WINDOW_SIZE if i % 6 != 0 else None)
...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained("meta-llama/Meta-Llama-3-8B", torch_dtype=torch.bfloat16,
                                                 device_map="auto", _attn_implementation="eager", use_cache=False)
