...
class Memcodes(nn.Module):
    def __init__(self, *, dim, num_codes, heads=8, temp=1.0):
        super().__init__()
    ...
    def forward(self, x, *, merge_output_heads=True):
        ...
        q = q * self.scale
        k, v = (
            self.to_k(self.codes),
            self.to_v(self.codes),
        )

        logits = einsum("b h i d, h j d -> b h i j", q, k)
        if self.training:
            attn = F.gumbel_softmax(logits, tau=self.temp, dim=-1, hard=True)
            codebook_indices = attn.softmax(dim=-1)
        else:
