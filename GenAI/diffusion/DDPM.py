...
#   author:proanimer
#   createTime:2024/2/17 上午12:29
#   lastModifiedTime:2024/2/17 上午12:29
#   file:DDPM.py
...
def linear_beta_schedule(timestamps):
    ...
    beta_end = scale * 0.02
    return torch.linspace(beta_start, beta_end, timestamps, dtype=torch.float64)
...
class WeightStandardizedConv2d(nn.Module):
    def forward(self, x):
        ...
        normalized_wight = (weight - mean) / (var + eps).rsqrt()
        return F.conv2d(
            x,
            normalized_wight,
            self.bias,
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None):
        ...
        h = self.block(x, scale_shift=scale_shift)
        h = self.block2(h)
    ...
    def __init__(
        ...
        *,
        ...
        if beta_schedule == "linear":
            ...
            raise ValueError(f"beta_schedule {beta_schedule} not found")
    ...
    def __init__(
        ...
        *,
        ...
        self.accelerator = Accelerator(
            split_batches=split_batches, mixed_precision="no"
        )
...
IMG_SIZE = (
    64  # Size of images, do not change this if you do not know why you need to change
)
batch_size = 16
...
dim_mults = (
    ...
    2,
    4,
)  # The model size will be (channels, 2 * channels, 4 * channels, 4 * channels, 2 * channels, channels)
...
diffusion = GaussianDiffusion(
    model, image_size=IMG_SIZE, timesteps=timesteps, beta_schedule=beta_schedule
)
